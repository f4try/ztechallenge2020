...
# 读入csv表格数据
data = pd.read_csv("Example0.csv", header=None)
...
def myCompute(label, n):
    for i in nodes:
        if i%n == label:
            for j in G.adj[i]:
                paths = list(nx.all_simple_paths(G, i, j, 10))
                for path in paths:
                    l = len(path)
                    path.sort()
...
# threads = []
# t1 = threading.Thread(target=myCompute(0, 1))
# threads.append(t1)
# t2 = threading.Thread(target=myCompute(1, 2))
# threads.append(t2)
# for t in threads:
#         t.setDaemon(True)
#         t.start()
