...
// index in path[] 
int Graph::printAllPathsUtil(int u, int d, bool visited[],
...
{
    //读文件
    ifstream inFile("test_data.txt",ios::in);//inFile来自fstream,ifstream为输入文件流(从文件读入)
    string lineStr;
    ...
    while(getline(inFile,lineStr)) //getline来自sstream
    {
        ...
        unordered_map<int,set<int> > map_record;
    ...
    cout << "Following are all different paths from " << s
         << " to " << d << endl;
    int result = g.printAllPaths(s, d);
    cout << result << endl;
